...
//int esMail()
//{
//	//int utn_getString(char* array,int lengArray,char* mensaje,char* eMensaje,int lowLimit,int intentos);
//}
//
//int utn_getMail(char* array,int lengArray,char* mensaje,char* eMensaje,int lowLimit,int intentos)
//{
//	int ret = -1;
...
//		{
//			if(myGets(buffer, lengArray) == 0 && esCharEspaciado(buffer) == 0)
//			{
//				utn_ToLowerString(buffer);
//				strcpy(array,buffer);
//				ret = 0;
//				break;
//			}
...
int esCharEspaciado(char *cadena)
{
	int ret = -1;
	int i;
	if (cadena!=NULL && strlen(cadena)>0)
	{
		for(i = 0; i < strlen(cadena); i++)
		{
			...
			{
				ret=0;	//Es un caracter alfabetico
			}
			else
			{
				ret = -1;//No es un caracter alfabetico, entonces rompo el for, fin de la funcion
				break;
			}
		}
	}
...
int utn_getNumeroMinimo(int* pResultado,char* mensaje,char* mensajeError,int minimo,int reintentos)
{
	int bufferInt;
	int ret = -1;
...
int utn_NombreDia(int numeroDia,char* nombreDia)
{
	int ret = -1;
	if(numeroDia > -1 && numeroDia < 8 && nombreDia != NULL)
	{
		switch (numeroDia)
		{
			case 0:
	...
	printf("\nNombre del dia es %s",nombreDia);
	return ret;
}
//********************************Validaciones archivos*********************************************
int utn_GetNombreArchivo(char* path,int tipoArchivo,int lengNombre)
{
	int ret = -1;
	fflush(stdin);
	if(utn_getString(path, lengNombre, "\nIngrese el nombre del archivo", "\nError, ingrese sólo nombre", 1, 3) == 0)
	{
		strcat(path,".");
		switch (tipoArchivo)
		{
