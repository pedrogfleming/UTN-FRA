...
int controller_addEmployee(LinkedList* pArrayListEmployee)
{
	...
	if(pArrayListEmployee != NULL)
	{
		...
		if(pAux != NULL)
		{
			if(idMax != -1)
			{
				if(employee_SetDatos(pAux) == 0)
				{
					idMax++;
					employee_setId(pAux,idMax);
					ll_add(pArrayListEmployee,pAux);
					printf("\nSe ha cargado con exito el nuevo empleado");
					controller_PrintEmployee(pAux);
					ret = 0;
				}
			}
		}
				...
				if(confirm == 's')
				{
					...
					switch (campoModificar)
					{
						case 1:
							...
							{
								if(employee_setNombre(auxP,nombre) == 0)
								{
									ret = 0;
						...
						case 2:
							...
							{
								if(employee_setHorasTrabajadas(auxP,horas) == 0)
								{
									ret = 0;
						...
						case 3:
							...
							{
								if(employee_setSueldo(auxP,sueldo) == 0)
								{
	...
	if(pArrayListEmployee != NULL)
	{
		...
		if(sizeLinkedList != -1)
		{
			for (i = 0; i < sizeLinkedList; i++)
			{
				...
				{
					employee_getId(empleadoMostrar,&auxId);
					employee_getNombre(empleadoMostrar, auxNombre);
					employee_getHorasTrabajadas(empleadoMostrar, &auxHorasTrabajadas);
					employee_getSueldo(empleadoMostrar, &auxSueldo);
		...
		if(ll_isEmpty(pArrayListEmployee) == 0)
		{
			...
			switch (criterioOrdenamiento)
			{
				...
				case 2: 	//Por horas trabajadas
					pOrdenar = sortByHoras;
				break;
				case 3: 	//Sueldo
					pOrdenar = sortBySueldo;
				break;
				case 4: 	//Por id
					pOrdenar = sortById;
					break;
				...
				for (i = 0; i < sizeLinkedList; i++)
				{
					...
					if(pAux != NULL)
					{
						if(		(employee_getId(pAux,&auxId)) != 0 ||
								(employee_getNombre(pAux,auxNombre)) != 0 ||
								(employee_getHorasTrabajadas(pAux,&auxHorasTrabajadas)) != 0 ||
								(employee_getSueldo(pAux,&auxSueldos)) != 0)
						{
							pAux = NULL;
							free(pAux);
...
void controller_PrintEmployee(Employee* index)
{
	if(index != NULL)
	{
		...
		int horasAux;
		int sueldoAux;
		employee_getId(index,&idAux);
		employee_getNombre(index,nombreAux);
		employee_getHorasTrabajadas(index,&horasAux);
		employee_getSueldo(index,&sueldoAux);
